-- ============================================================
-- 企业级 AI 知识管理平台 - 乱码修复脚本
-- 数据库: MySQL 8.x
-- 目标字符集: utf8mb4 / utf8mb4_unicode_ci
--
-- 重要提醒：
--   1. 执行前请务必确认数据库客户端的【连接编码】为 UTF-8：
--      Navicat: 连接属性 -> 高级 -> 编码 -> UTF-8
--      否则重新执行本脚本后依然会乱码。
--   2. 本脚本会 DROP 并重建整个 enterprise_ai 数据库，
--      所有旧数据将被清空（按你的确认，可清空重建）。
--   3. 下面的 SET 语句保证本次会话按 utf8mb4 写入。
-- ============================================================
...
-- 删除旧库（彻底清除乱码表和数据）
DROP DATABASE IF EXISTS enterprise_ai;
...
UNION ALL
SELECT '知识库表', COUNT(*) FROM knowledge_base
UNION ALL
SELECT '文档表', COUNT(*) FROM document;

-- 检查中文是否正常显示（应显示：超级管理员 / 普通员工）
SELECT id, role_name, role_code, description FROM sys_role;
