...
-- 初始化系统配置
-- 仅保留真实生效的配置键（system.name / system.description 供登录页、侧边栏、浏览器标题使用；system.icp 供登录页底部展示）
-- 存储/安全/AI 配置此前存了但代码从不读取（读 application.yml + 硬编码），已从设置页删除，故 seed 一并移除
